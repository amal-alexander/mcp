...
async function detectConfig() {
    ...
    return {
        ...
        adobeAccounts: accounts.filter(a => a.engine === 'adobe'),
...
function printDetectionSummary(results: any) {
    ...
    const adobeCount = results.adobeAccounts ? results.adobeAccounts.length : 0;

    if (gCount === 0 && bCount === 0 && ga4Count === 0 && adobeCount === 0) return;
    ...
    printStatusLine('Adobe Analytics', adobeCount > 0);
    console.log('');
}
...
async function setupServiceAccount() {
    ...
    showMcpConfigSnippet();
    console.log('\n🎉 Setup complete! You can now use MCP by Amal Alexander.\n');
    ...
    if (answer === '' || answer.toLowerCase().startsWith('y')) {
        try {
            ...
            if (repo && /^[a-zA-Z0-9_.-]+\/[a-zA-Z0-9_.-]+$/.test(repo)) {
                ...
                console.log('🔗 https://github.com/amal-alexander/search-console-mcp');
            }
        } catch (error) {
            console.log('🔗 https://github.com/amal-alexander/search-console-mcp');
        }
...
export async function main() {
    ...
    if (engineFlag === 'bing') {
        ...
        return;
    } else if (engineFlag === 'adobe') {
        await handleAdobeFlow(configStatus);
        return;
    }
...
async function checkAndShowAdobeSites(configStatus: any): Promise<boolean> {
    const isConnected = configStatus.adobeAccounts && configStatus.adobeAccounts.length > 0;
    if (!isConnected) return true;

    console.log(`${colors.green}✔ Adobe Analytics is already connected!${colors.reset}`);
    const config = await loadConfig();
    const accounts = Object.values(config.accounts).filter(a => a.engine === 'adobe');

    console.log(`\n${colors.bold}Your configured Adobe Analytics accounts:${colors.reset}`);
    accounts.forEach(a => console.log(`  • ${a.alias} (Report Suite: ${a.adobeReportSuiteId})`));

    const reconf = await ask(`\nWould you like to reconfigure Adobe Analytics? (y/N): `);
    return reconf.toLowerCase().startsWith('y');
}

async function handleAdobeFlow(configStatus: any) {
    const shouldProceed = await checkAndShowAdobeSites(configStatus);
    if (!shouldProceed) {
        console.log(`\n${colors.green}✔${colors.reset} ${colors.bold}Configuration untouched. You're ready to roll!${colors.reset}`);
        return;
    }
...
async function setupAdobe() {
    printBoxHeader('Adobe Analytics Setup');

    printStep(1, 'Create an Adobe API Credential (OAuth Server-to-Server)');
    console.log('If you don\'t have credentials yet, follow these steps:');
    console.log('  1. Go to https://developer.adobe.com/console/projects');
    console.log('  2. Create a new Project (or open an existing one)');
    console.log('  3. Click "Add to Project" > "API" > "Adobe Analytics"');
    console.log('  4. Select "OAuth Server-to-Server" and click "Save configured API"');
    console.log('  5. From the credential page, copy your:');
    console.log('     - Client ID');
    console.log('     - Client Secret');
    console.log('     - IMS Organization ID (shown in the top-right of console.adobe.com)');
    console.log('  6. Ensure the product profile linked has access to your Report Suites\n');

    const clientId = await ask('Enter your Adobe Client ID: ');
    if (!clientId) { printError('Client ID is required.'); return; }

    const clientSecret = await ask('Enter your Adobe Client Secret: ');
    if (!clientSecret) { printError('Client Secret is required.'); return; }

    const orgId = await ask('Enter your IMS Organization ID (e.g. XXXXXXXX@AdobeOrg): ');
    if (!orgId) { printError('IMS Org ID is required.'); return; }

    printStep(2, 'Enter your Report Suite ID');
    console.log('You can find your Report Suite IDs in:');
    console.log('  Adobe Analytics > Admin > Report Suites\n');

    const reportSuiteId = await ask('Enter your Report Suite ID: ');
    if (!reportSuiteId) { printError('Report Suite ID is required.'); return; }

    printStep(3, 'Verify Credentials');
    printInfo('Attempting to verify your Adobe Analytics credentials...');

    let verified = false;
    try {
        // Get an access token using OAuth Server-to-Server
        ...
        if (tokenResponse.ok) {
            printSuccess('Adobe OAuth token obtained successfully!');
            verified = true;
        } else {
            const errBody = await tokenResponse.text();
            printError(`Token request failed (${tokenResponse.status}): ${errBody}`);
            const proceed = await ask('Credentials could not be verified. Save anyway? (y/N): ');
            verified = proceed.toLowerCase().startsWith('y');
        }
    } catch (e) {
        printInfo(`Could not reach Adobe IMS endpoint: ${(e as Error).message}`);
        const proceed = await ask('Save credentials anyway? (y/N): ');
        verified = proceed.toLowerCase().startsWith('y');
    }

    if (!verified) {
        printError('Adobe Analytics setup aborted.');
        return;
    }

    const alias = await ask(`Enter an alias for this account (optional, default: Adobe-${reportSuiteId}): `) || `Adobe-${reportSuiteId}`;

    const account: AccountConfig = {
        id: `adobe_${Date.now()}`,
        engine: 'adobe',
        alias,
        adobeClientId: clientId,
        adobeClientSecret: clientSecret,
        adobeOrgId: orgId,
        adobeReportSuiteId: reportSuiteId,
        websites: [reportSuiteId]
    };

    await updateAccount(account);
    printSuccess(`Successfully added Adobe Analytics account "${alias}"!`);

    printStep(4, 'Configure your MCP client');
    showMcpConfigSnippet();
    console.log('\n🎉 Setup complete! You can now use Adobe Analytics in MCP by Amal Alexander.\n');
