...
server.tool(
  "seo_keyword_clustering",
  "Semantic keyword clustering tool to group search queries by topic/tokens and analyze metrics",
  {
    siteUrl: z.string().describe("The site URL"),
    days: z.number().optional().describe("Number of days (default: 28)"),
    minImpressions: z.number().optional().describe("Minimum impressions threshold (default: 5)"),
    limit: z.number().optional().describe("Max query rows to analyze (default: 1000)"),
    ...
    })).optional().describe("Optional list of custom queries to cluster")
  },
  async ({ siteUrl, days, minImpressions, limit, customQueries }) => {
    try {
      const result = await seoInsights.clusterKeywords(siteUrl, { days, minImpressions, limit }, customQueries);
      return {
        content: [{ type: "text", text: JSON.stringify(result, null, 2) }]
      };
    } catch (error) {
...
server.tool(
  "seo_keyword_intent_classifier",
  "Classify search queries into Search Intent categories (Informational, Transactional, Navigational, Commercial)",
  {
    siteUrl: z.string().describe("The site URL"),
    brandRegex: z.string().optional().describe("Optional brand regex to classify navigational queries"),
    days: z.number().optional().describe("Number of days (default: 28)"),
    minImpressions: z.number().optional().describe("Minimum impressions threshold (default: 5)"),
    limit: z.number().optional().describe("Max query rows to analyze (default: 1000)")
  },
  async ({ siteUrl, brandRegex, days, minImpressions, limit }) => {
    try {
      const result = await seoInsights.classifyKeywordIntent(siteUrl, { days, minImpressions, limit, brandRegex });
      return {
        content: [{ type: "text", text: JSON.stringify(result, null, 2) }]
      };
    } catch (error) {
...
async function main() {
  ...
  console.error(`🚀 ${colors.bold}${colors.green}MCP by Amal Alexander${colors.reset} ${colors.dim}| GSC · GA4 · Bing${colors.reset}`);
  console.error(`${colors.bold}${colors.cyan}======================================================${colors.reset}\n`);
  console.error(`MCP by Amal Alexander — running on stdio [ ${googleStatus} | ${ga4Status} | ${bingStatus} ]`);
}
