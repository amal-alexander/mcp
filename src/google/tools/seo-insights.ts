...
/**
 * Keyword Cluster representing a group of semantically similar queries.
 */
export interface KeywordCluster {
    /** The core token defining this cluster. */
    name: string;
    /** The search queries grouped under this cluster. */
    queries: string[];
    /** Combined clicks for this cluster. */
    clicks: number;
    /** Combined impressions for this cluster. */
    impressions: number;
    /** Average click-through rate. */
    ctr: number;
    /** Average ranking position of keywords in the cluster. */
    position: number;
}

/**
 * Group search queries into semantic clusters based on token overlaps.
 *
 * @param siteUrl - The URL of the site to analyze.
 * @param options - Configuration including lookback period and volume thresholds.
 * @param customQueries - Optional list of custom queries to cluster instead of GSC.
 * @returns Semantically grouped keyword clusters with aggregated metrics.
 */
export async function clusterKeywords(
    siteUrl: string,
    options: { days?: number; minImpressions?: number; limit?: number } = {},
    customQueries?: Array<{ query: string; clicks?: number; impressions?: number; position?: number; ctr?: number }>
): Promise<KeywordCluster[]> {
    const { days = 28, minImpressions = 5, limit = 1000 } = options;

    let validRows: Array<{ query: string; clicks: number; impressions: number; position: number; ctr: number }> = [];

    if (customQueries && customQueries.length > 0) {
        validRows = customQueries.map(q => ({
            query: q.query,
            clicks: q.clicks ?? 0,
        ...
        validRows = rows
            .filter(r => (r.impressions ?? 0) >= minImpressions && r.keys?.[0])
            .map(r => ({
                query: r.keys![0],
                clicks: r.clicks ?? 0,
                impressions: r.impressions ?? 0,
    ...
    const stopwords = new Set([
        'the', 'and', 'a', 'of', 'to', 'in', 'is', 'for', 'on', 'with', 'at', 'by', 'an', 'this', 'that', 'from', 'or', 'as', 'your', 'my'
    ]);
    ...
    for (const r of validRows) {
        ...
        const tokens = query.split(/\s+/).map(t => t.replace(/[^a-z0-9]/g, '')).filter(t => t.length > 2 && !stopwords.has(t));
        queryTokens.set(query, tokens);

        for (const token of tokens) {
            tokenCounts.set(token, (tokenCounts.get(token) || 0) + r.impressions);
        }
    }
    ...
    for (const t of topTokens) {
        clustersMap.set(t, { queries: [], clicks: 0, impressions: 0, weightedPos: 0 });
    }
    const miscKey = 'other / informational';
    clustersMap.set(miscKey, { queries: [], clicks: 0, impressions: 0, weightedPos: 0 });

    for (const r of validRows) {
        const query = r.query.toLowerCase().trim();
        const tokens = queryTokens.get(query) || [];
        
        let assigned = false;
        for (const t of topTokens) {
            if (tokens.includes(t)) {
                const cluster = clustersMap.get(t)!;
                cluster.queries.push(r.query);
                cluster.clicks += r.clicks;
                cluster.impressions += r.impressions;
                cluster.weightedPos += r.position * r.impressions;
                assigned = true;
                break;
            }
        }
        ...
        if (!assigned) {
            const cluster = clustersMap.get(miscKey)!;
            cluster.queries.push(r.query);
            cluster.clicks += r.clicks;
            cluster.impressions += r.impressions;
            cluster.weightedPos += r.position * r.impressions;
    ...
    for (const [name, data] of clustersMap.entries()) {
        ...
        clusters.push({
            ...
            ctr: data.impressions ? data.clicks / data.impressions : 0,
            position: data.impressions ? data.weightedPos / data.impressions : 0
        });
    }
...
/**
 * Search Intent Metrics showing performance split by intent type.
 */
export interface IntentMetrics {
    /** The classified search intent. */
    intent: 'Informational' | 'Commercial' | 'Transactional' | 'Navigational' | 'Unknown';
    /** Combined clicks for the intent group. */
    clicks: number;
    /** Combined impressions for the intent group. */
    impressions: number;
    /** Average click-through rate. */
    ctr: number;
    /** Average ranking position. */
    position: number;
    /** Total number of unique queries in this intent. */
    queryCount: number;
    /** Sample queries matching this intent. */
    queries: string[];
}

/**
 * Classify search console queries into intent categories and analyze performance metrics.
 *
 * @param siteUrl - The URL of the site to analyze.
 * @param options - Configuration including lookback period, brand regex, and thresholds.
 * @returns Metrics split across Informational, Commercial, Transactional, and Navigational intents.
 */
export async function classifyKeywordIntent(
    siteUrl: string,
    options: { days?: number; minImpressions?: number; limit?: number; brandRegex?: string } = {}
): Promise<IntentMetrics[]> {
    const { days = 28, minImpressions = 5, limit = 1000, brandRegex = '' } = options;
    const endDate = new Date();
    endDate.setDate(endDate.getDate() - 3);
    const startDate = new Date(endDate);
    startDate.setDate(startDate.getDate() - days);
    ...
    const validRows = rows.filter(r => (r.impressions ?? 0) >= minImpressions && r.keys?.[0]);

    const informationalPatterns = /\b(how|what|why|when|who|where|guide|tutorial|tips|ideas|example|examples|pdf|free|learn|difference|meaning|definition|course|tutorial|instructions|methods|steps|prevent|fix|repair|troubleshoot)\b/i;
    const commercialPatterns = /\b(best|review|reviews|top|vs|compare|alternative|alternatives|comparison|software|tool|rating|ratings|recommended)\b/i;
    const transactionalPatterns = /\b(buy|purchase|order|cheap|pricing|price|coupon|coupons|shop|store|sale|discount|promo|hire|agency|services|cost|checkout|subscribe)\b/i;
    const navigationalPatterns = /\b(login|signin|sign\sin|contact|support|app|website|official|phone\snumber|portal|download|home|careers|jobs|helpdesk)\b/i;

    const intentGroups: Record<string, {
        clicks: number;
        impressions: number;
        weightedPos: number;
        queryCount: number;
        ...
        Informational: { clicks: 0, impressions: 0, weightedPos: 0, queryCount: 0, queries: [] },
        Commercial: { clicks: 0, impressions: 0, weightedPos: 0, queryCount: 0, queries: [] },
        Transactional: { clicks: 0, impressions: 0, weightedPos: 0, queryCount: 0, queries: [] },
        Navigational: { clicks: 0, impressions: 0, weightedPos: 0, queryCount: 0, queries: [] },
        Unknown: { clicks: 0, impressions: 0, weightedPos: 0, queryCount: 0, queries: [] }
    };

    let brandRe: RegExp | null = null;
    if (brandRegex) {
        try {
            brandRe = new RegExp(brandRegex, 'i');
        } catch (e) {
    ...
    for (const r of validRows) {
        ...
        let intent: keyof typeof intentGroups = 'Unknown';
        
        if (brandRe && brandRe.test(query)) {
            intent = 'Navigational';
        } else if (transactionalPatterns.test(query)) {
            intent = 'Transactional';
        } else if (commercialPatterns.test(query)) {
            intent = 'Commercial';
        } else if (informationalPatterns.test(query)) {
            intent = 'Informational';
        } else if (navigationalPatterns.test(query)) {
            intent = 'Navigational';
        }
        ...
        group.weightedPos += (r.position ?? 0) * (r.impressions ?? 0);
        group.queryCount++;
