...
const SERVICE_NAME = 'io.github.amal-alexander.search-console-mcp';
const DEFAULT_ACCOUNT = 'default';

// Default Client ID for Desktop Flow — set via environment variables
// Copy .env.example to .env and fill in your values, or set them in your Claude Desktop config env block
...
export async function startLocalFlow(clientId: string, clientSecret: string, scopes: string[] = SCOPES): Promise<any> {
  ...
  return new Promise((resolve, reject) => {
    ...
    console.log('\nOpening your browser to authorize Google access (GSC + GA4)...');
    open(authUrl);
  });
}
